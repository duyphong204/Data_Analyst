...
def get_sp500_tickers():
    ...
    headers = {"User-Agent": "Mozilla/5.0"}  # giả lập trình duyệt thật
...
def tab1():
    ...
    with c1:        
        if ticker != '-':
            ...
            # Nếu summary là dict → chuyển sang DataFrame
            if isinstance(summary, dict):
                summary_df = pd.DataFrame(list(summary.items()), columns=['Attribute', 'Value'])
            else:
                summary_df = summary  # Trường hợp getsummary() trả về DataFrame

            st.subheader("Company Summary (Part 1)")
            st.dataframe(summary_df)

            # Nếu muốn chọn ra một số dòng cụ thể (ví dụ 8 dòng đầu tiên)
            showsummary = summary_df.iloc[:8, :]  # tránh lỗi index
            showsummary.set_index('Attribute', inplace=True)
            st.dataframe(showsummary)
    ...
    with c2:        
        if ticker != '-':
            ...
            st.subheader("Company Summary (Part 2)")
            st.dataframe(summary_df)

            # Lấy 8 dòng tiếp theo nếu có
            if len(summary_df) > 8:
                showsummary = summary_df.iloc[8:16, :]
                showsummary.set_index('Attribute', inplace=True)
                st.dataframe(showsummary)
            else:
                st.info("Không đủ dữ liệu để hiển thị phần 2.")
    ...
    if ticker != '-':
            ...
            if not chartdata.empty and 'Close' in chartdata.columns:
                fig = px.area(chartdata, x=chartdata.index, y=chartdata['Close'].squeeze(), title=f"{ticker} Closing Price Over Time")
